...
selectedAirports = ["LFPG"]

def get_files(search_path):
    return [f for f in Path(dataPath).glob('**/*.gz') if f.is_file()]


def return_icao_code(code_total):
    return code_total[1:-1].split("/")[1]

def readMongoData(selectedAirports):
    airport_list = []

    for post in airports.find():
        pyairports.append(post['airport'])
        airport_code_icao = return_icao_code(post['airport']['code_total'])
        if airport_code_icao in selectedAirports:
            print("Found", airport_code_icao)
            airport_list.append(post['airport'])
...
for f in get_files(dataPath):
    ...
    readMongoData(selectedAirports)
